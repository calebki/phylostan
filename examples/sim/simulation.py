...
import os
import sys
sys.path.insert(0,'../../phylostan')
import utils
import numpy as np

epi = 'sim'
stem = f'' #Stem for files
...
tree_prior = 'bdsky'
_script = f'vbsky_fix_rate.stan' #STAN script files
...
if _clock is not None:
    if tree_prior == 'skygrid':
        ...
        data['x1'] = 0
        # data['delta'] = np.repeat(0.3, 10)
        # data['s'] = np.repeat(0.1, 10)
...
for seed in _seeds:
    sample_path = f'output/{stem}{epi}_{seed}'
